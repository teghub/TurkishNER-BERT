...
#gpuda calisacaksan gerekli kodlar
...
class DataProcessor(object):
    ...
    def _read_data(cls,input_file):
        ...
        for line in rf:
            ...
            # verisetimizdeki bütün cümleler nokta ile bitmediği icin words[-1] == '.' condition'i kaldirildi
            if len(line.strip())==0:
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        :return:
        """
        # comment icindeki satir bert'in kendi verisetinin labellari
        # acik olan satir bizim verisetimizin labellari
        # farkli bir veriseti ile calisilirsa return value güncellenmeli

        #return ["[PAD]","B-MISC", "I-MISC", "O", "B-PER", "I-PER", "B-ORG", "I-ORG", "B-LOC", "I-LOC", "X","[CLS]","[SEP]"]
        return ["[PAD]","ORGANIZATION", "LOCATION", "PERSON", "MONEY", "DATE", "TIME", "PERCENT", "O", "X","[CLS]","[SEP]","A"]
...
def _write_base(batch_tokens,id2label,prediction,batch_labels,wf,i):
    ...
    #turkce karakterlerle ilgili sıkıntıyı gidermek icin utf-8 encoding 
    token = batch_tokens[i].encode("utf-8")
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        with open(output_eval_file,"w") as wf:
            ...
            # eval_results.txt'ye yazmıyordu, yazması icin
            wf.write("precision = %s\n" % p)
            wf.write("recall = %s\n" % r)
            wf.write("f_score = %s\n" % f)
            logging.info("***********************************************")
